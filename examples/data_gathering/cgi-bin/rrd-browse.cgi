...
#	for (qw(txt html css xsl xslt xml)) {
#		$include{"include_$_"} = "$dir{data}/$host/include.$_"
#			if -e "$dir{data}/$host/include.$_";
#	}

	if (!grep(/^$host$/,@graphs)) {
		push @{$tmpl_cache->{hosts}}, { host => $host, no_graphs => 1, %include };

	} else {
		my %host = ( host => $host, %include );
		for (qw(thumbnails graphs)) {
			eval {
				...
				for my $img (sort alpha_period 
					...
					my $stale = stale_rrd("$dir{data}/$host/$graph.rrd");
					$hash{stale} = $stale if defined($stale);
					...
					$hash{txt} = "$dir{graphs}/$host/$img.txt"
						if $_ eq 'graphs' && -e "$dir{graphs}/$host/$img.txt";
...
sub stale_rrd {
	...
	return scalar(localtime($rrd_mtime)) if time - $rrd_mtime >= STALE_THRESHOLD;
	return;
}
